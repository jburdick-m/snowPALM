...
model_pars = {
    ...
    'snow_mult': 1.03,                 # Snowfall Multiplier [-]
    'srad_mult_summer': 1.05,       # Shortwave Radiation Multiplier (summer) [-]
    'srad_mult_winter': 1.05,       # Shortwave Radiation Multiplier (winter) [-]
    'lrad_mult_summer': 1.05,       # Longwave Radiation Multiplier (summer) [-]
    'lrad_mult_winter': 1.05,       # Longwave Radiation Multiplier (winter) [-]
    ...
    'longwave_trans': 0.1,          # Transmittance factor for longwave radiation under canopy [-] (suggested range: 0 - 0.5)
    'shortwave_trans_diffuse': 0.1, # Transmittance factor for diffuse shortwave radiation under canopy [-] (suggested range: 0 - 0.5)
    'shortwave_trans_direct': 0.1, 	# Transmittance factor for direct shortwave radiation under canopy [-] (suggested range: 0 - 0.2)
    'lwi_par1': 0.5,                # LWI (Or longwave influence of warm tree trunks at the edges of canopy stands) Parameter 1 (suggested range: 0.5 - 1.5)
    ...
    'snowfall_wind_exp': 1,         # Exponent applied to wind index map (suggested range: 0.5 - 2), only if wind model is on
    ...
    'MaxAlbedo': 0.90,              # Maximum (or fresh snow) albedo (-) (suggested range: 0.8 - 0.9)
    'minalbedo': 0.35,              # Minimum snowpack albedo [-] (suggested range: 0.3 - 0.4)
    'albedo_decay_cold': 0.02,      # Albedo Decay Factor for cold snowpack [1/day] (suggested range: 0.01 - 0.05)
    'albedo_decay_warm': 0.1,       # Albedo Decay Factor for cold snowpack [1/day] (suggested range: 0.05 - 0.15)
    ...
    'apar': 0.015,                  # Snow densification rate due to age [Fraction / day] (suggested range: 0.01-0.02)
    'dpar': 0.0015,                 # Snow densfication rate due to overburdin [Fraction / cm SWE] (suggested range: 0.001-0.002)
    ...
    'melt_drip_par': 0.3,           # Melt Drip Rate [mm/day per deg-C above freezing] (suggested range: 0-1)
    'snow_unload_par': 0.1,         # Fraction of canopy snow that unloads each day [-] (suggested range: 0-1)
    'canopy_sub_mult': 0.25,          # Canopy sublimation multiplier applied to potential sublimation rate (which is computed for snowpack surface) [-] (suggested range: 5-20)
    ...
    'max_sub_pct_when_snow': 0.005,   # Maximum fraction of snowfall that can be sublimation when snowing (suggested range: 0-0.5)
    'max_melt_pct_when_snow': 0.005,  # Maximum fraction of snowfall that can be melt when snowing (suggested range: 0-0.5)
    ...
    'sroughness': 5E-5,             # Snow surface roughness length [m] (suggested range: 1E-5 - 1E-4)
